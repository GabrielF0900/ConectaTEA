    ...
    <div className="h-full">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <div className="px-2 py-1 text-sm bg-green-100 text-green-600 rounded">PROFISSIONAL</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { titulo: "Total de Crianças", valor: "24", detalhe: "+2 este mês", cor: "blue" },
            { titulo: "Profissionais Ativos", valor: "8", detalhe: "+1 este mês", cor: "green" },
            { titulo: "Metas Ativas", valor: "47", detalhe: "+5 esta semana", cor: "purple" },
            { titulo: "Taxa de Progresso", valor: "78%", detalhe: "+12% este mês", cor: "orange" }
          ].map((c, i) => (
            <div key={i} className="bg-white rounded-lg shadow-sm border p-4 flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-gray-500">{c.titulo}</span>
                <div className={`w-8 h-8 rounded-full bg-${c.cor}-100 flex items-center justify-center`}>
                  <svg className={`w-4 h-4 text-${c.cor}-600`} fill="currentColor" viewBox="0 0 20 20">
                    <circle cx="10" cy="10" r="5" />
                  </svg>
                </div>
              </div>
              <span className="text-3xl font-bold mb-1">{c.valor}</span>
              <span className="text-xs text-gray-400">{c.detalhe}</span>
            </div>
          ))}
        </div>
                ...
                <div key={i} className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`bg-${c.cor}-100 text-${c.cor}-600 text-xs px-2 py-1 rounded`}>
                        {c.status}
                      </span>
                    </div>
                ...
                <div key={i}>
                  <div className="flex justify-between items-center mb-2">
                    <div>
                      ...
                      <span className={`bg-${m.cor}-100 text-${m.cor}-600 text-xs px-2 py-1 rounded ml-2`}>
                        {m.status}
                      </span>
                    </div>
                    <span className="text-sm text-gray-500">{m.progresso}%</span>
                  </div>
                  <div className="text-sm text-gray-500 mb-2">{m.descricao}</div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className="bg-green-500 h-2 rounded-full" style={{ width: `${m.progresso}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
