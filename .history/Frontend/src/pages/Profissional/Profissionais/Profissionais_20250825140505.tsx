...
export default function Profissionais() {
  ...
  const fetchProfissionais = useCallback(async (search?: string) => {
    try {
      ...
      const filtrados = mapeados.filter((p) => {
        if (loggedUserId && p.usuario_id && p.usuario_id === loggedUserId) return false;
  ...
  function getRedeUrl(prof: Profissional, tipoBusca: string) {
    ...
    if (prof.redes && typeof prof.redes === 'object') {
      ...
      const val = (prof.redes as Record<string, string | null>)[keyLower] ?? (prof.redes as any)[tipoBusca];
      if (val) return val;
    }
    // checa array de redes (redesArray)
    if ((prof as any).redesArray && Array.isArray((prof as any).redesArray)) {
      const found = (prof as any).redesArray.find((r: any) => r?.tipo && String(r.tipo).toLowerCase().includes(keyLower));
      if (found) return found.url ?? null;
    }
    // checa redes sociais relacionais (redesSociais) formato backend
    if ((prof as any).redesSociais && Array.isArray((prof as any).redesSociais)) {
      const found = (prof as any).redesSociais.find((r: any) => r?.tipo && String(r.tipo).toLowerCase().includes(keyLower));
      if (found) return found.url ?? null;
    }
    return null;
  }
