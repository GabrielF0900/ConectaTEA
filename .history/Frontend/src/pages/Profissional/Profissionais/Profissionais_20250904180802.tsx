  ...
  const handleAceitar = async (prof: Profissional) => {
    ...
    try {
      ...
      const conexaoParaAceitar = conexoes.find(
        c => c.solicitante_id === prof.id && c.solicitado_id === loggedProfissionalId && c.status === 'PENDENTE'
      );

      if (!conexaoParaAceitar) {
        notificarErro('Erro', 'Não foi possível encontrar a solicitação para aceitar.');
        return;
      }
      ...
      const [recebidas, enviadas] = await Promise.all([
        listarSolicitacoesRecebidas(loggedUserId || 0),
        listarSolicitacoesEnviadas(loggedUserId || 0),
      ]);
  ...
  const handleRecusar = async (prof: Profissional) => {
    ...
    try {
      ...
      if (!conexaoParaRecusar) {
        notificarErro('Erro', 'Não foi possível encontrar a solicitação para recusar.');
        return;
      }
      ...
      const novasConexoes = conexoes.filter(c => c.id !== conexaoParaRecusar.id);
    ...
    try {
      ...
      if (loggedUserId) {
        ...
        if (loggedProfissionalId) {
          ...
          const conexoesAceitas = await listarConexoesPorProfissional(loggedProfissionalId);
          
          // Armazena todas as conexões (pendentes + aceitas)
          const todasConexoes = [...recebidas, ...enviadas, ...conexoesAceitas];
          setConexoes(todasConexoes);
          
          const connectedIds = new Set(conexoesAceitas.flatMap(c => [c.solicitante_id, c.solicitado_id]));
          setProfissionais(prev => prev.map(p => ({ ...p, conectado: connectedIds.has(p.id) } as Profissional)));
        } else {
                    ...
                    <div className="flex items-center gap-2 bg-green-500 text-white rounded-lg px-4 py-2">
                      ...
                      <button title="Desfazer conexão" onClick={async () => {
                        try {
                          ...
                          console.log('Usuário logado ID:', loggedProfissionalId);
                          console.log('Profissional alvo ID:', prof.id);
                          
                          // Encontra a conexão aceita para remover
                          const conexaoParaRemover = conexoes.find(
                            c => {
                              console.log('Verificando conexão:', c);
                              return ((c.solicitante_id === loggedProfissionalId && c.solicitado_id === prof.id) ||
                                      (c.solicitante_id === prof.id && c.solicitado_id === loggedProfissionalId)) &&
                                     c.status === 'ACEITO'
                            }
                          );

                          console.log('Conexão para remover:', conexaoParaRemover);

                          if (!conexaoParaRemover) {
                            notificarErro('Erro', 'Não foi possível encontrar a conexão para remover.');
                            return;
                          }
                          ...
                          setConexoes(prev => prev.filter(c => c.id !== conexaoParaRemover.id));
