...
export default function Profissionais() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fb] py-6 px-2 sm:px-0">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Networking Profissional</h1>
          <p className="text-gray-500 text-lg mt-1">Conecte-se com outros profissionais de saúde</p>
        </div>

        {/* Abas */}
        <div className="flex gap-2 mb-6">
          <button
            className={`px-5 py-2 rounded-lg font-semibold text-sm transition border ${abaAtiva === 'todos' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'}`}
            onClick={() => setAbaAtiva('todos')}
          >
            Todos os Profissionais
          </button>
          <button
            className={`px-5 py-2 rounded-lg font-semibold text-sm transition border ${abaAtiva === 'conexoes' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'}`}
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-6 items-center">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
            />
          </div>
            ...
            >
              <div className="flex flex-col gap-2 mb-2">
                ...
                <div className="flex gap-2 items-center">
                  <span className="bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded">{prof.especialidade}</span>
                  <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-0.5 rounded">{prof.status}</span>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {prof.areas.map((area, idx) => (
                    <span key={idx} className="bg-gray-100 text-gray-700 text-xs font-medium px-2 py-0.5 rounded-full border border-gray-200">{area}</span>
                  ))}
                </div>
              </div>
