...
const NotificationItem: React.FC<{
  ...
  }, [notification.duration, handleRemove]); // Agora handleRemove é estável
  ...
  return (
    <div
      className={`
        ...
        ${isRemoving ? 'scale-95 opacity-0 -translate-y-2' : ''}
      `}
    >
      <div className={`
        w-full ${config.bgColor} border rounded-lg shadow-lg p-4
...
const BarraDeNotificacao: React.FC<BarraDeNotificacaoProps> = ({
  ...
  return createPortal(
    <div 
      className={`fixed z-[9999] pointer-events-none ${getPositionClasses(position)}`}
      style={{ 
        maxWidth: '420px',
        minWidth: '320px'
      }}
