...
export async function listarProfissionais(params?: { usuarioId?: number; search?: string }): Promise<Profissional[]> {
	try {
		...
		const { data } = await api.get<Profissional[]>(`/api/profissionais${suffix}`);
		return data;
	} catch (e) {
...
export async function obterProfissionalPorId(id: number): Promise<Profissional> {
	try {
		const { data } = await api.get<Profissional>(`/api/profissionais/${id}`);
		return data;
	} catch (e) {
		throw new Error(extractErrorMessage(e));
	}
...
export async function obterProfissionalPorUsuarioId(usuarioId: number): Promise<Profissional | null> {
	try {
		const { data } = await api.get<Profissional[]>(`/api/profissionais?usuarioId=${encodeURIComponent(usuarioId)}`);
...
export async function atualizarPerfilUsuario(usuarioId: number, payload: Partial<Profissional>): Promise<Profissional> {
	try {
		const { data } = await api.put<Profissional>(`/api/private/atualizar-perfil/${encodeURIComponent(usuarioId)}`, payload);
		return data;
	} catch (e) {
		throw new Error(extractErrorMessage(e));
	}
