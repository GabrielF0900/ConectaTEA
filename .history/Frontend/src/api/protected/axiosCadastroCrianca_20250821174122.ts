...
export const excluirCrianca = async (criancaId: number): Promise<{ message: string }> => {
  try {
    const response = await api.delete<{ message: string }>(`/private/delete/${criancaId}`);
    return response.data;
  } catch (error) {
...
export const buscarCriancaPorId = async (criancaId: number): Promise<CriancaListagem> => {
  try {
    const response = await api.get<{ crianca: CriancaListagem }>(`/private/read/${criancaId}`);
    return response.data.crianca;
  } catch (error) {
...
export const atualizarCrianca = async (criancaId: number, data: AtualizarCriancaData): Promise<{ message: string; crianca: CriancaListagem }> => {
  try {
    const response = await api.put<{ message: string; crianca: CriancaListagem }>(`/private/update/${criancaId}`, data);
    return response.data;
  } catch (error) {
