	...
	try {
		...
		if (userData.data) {
			...
			return {
				...
				locais: [
					{ id: 1, nome: "Consultório Central", cidade: "São Paulo" },
					{ id: 2, nome: "Clínica TEA+", cidade: "Salvador" }
				],
				redesSociais: [
					{ id: 1, tipo: "linkedin", url: "https://linkedin.com/in/exemplo" }
				],
				redes: {
...
export async function atualizarPerfilProfissional(usuarioId: number, payload: Partial<Profissional>): Promise<Profissional> {
	try {
		const { data } = await api.put<{ message: string; data: Profissional }>(`/profissionais/usuario/${encodeURIComponent(usuarioId)}`, payload);
		return data.data;
	} catch (e) {
		throw new Error(extractErrorMessage(e));
	}
