...
export async function CadastrarCrianca(req: Request, res: Response) {
  ...
  try {
    ...
    let responsavel = await prisma.user.findFirst({
      where: {
        AND: [
          { name: name },
          { telefone: phone },
          email ? { email: email } : { email: { equals: null } },
          { tipo: "RESPONSAVEL" }
        ]
      },
    });
    ...
    if (!responsavel) {
      console.log("📝 Responsável não encontrado com dados idênticos, criando novo...");
      
      // Criar novo responsável sempre que os dados não baterem 100%
      const userData: any = {
        name,
        telefone: phone,
