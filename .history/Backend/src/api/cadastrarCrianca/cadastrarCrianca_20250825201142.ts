...
export async function CadastrarCrianca(req: Request, res: Response) {
  ...
  // Validar telefone (formatos aceitos: 71997209361, (71) 99720-9361, 71 99720-9361)
  const phoneFormats = [
    /^\(\d{2}\)\s?\d{4,5}-?\d{4}$/, // (71) 99720-9361 ou (71)997209361
    /^\d{2}\s?\d{4,5}-?\d{4}$/, // 71 99720-9361 ou 71 997209361
    /^\d{10,11}$/ // 71997209361 (só números)
  ];
  ...
  if (!isValidPhone) {
    ...
    console.log("Formatos aceitos: (71) 99720-9361, 71 99720-9361, ou 71997209361");
    return res.status(400).json({ 
      message: "Telefone deve estar em um dos formatos: (71) 99720-9361, 71 99720-9361, ou 71997209361." 
    });
  }
  ...
  try {
    ...
    try {
      ...
      if (err.code === 'P2002' && err.meta && err.meta.target && err.meta.target.includes('email')) {
        return res.status(400).json({
          message: 'Já existe um responsável cadastrado com este e-mail. Por favor, utilize outro e-mail ou recupere o acesso.'
        });
      }
      // Outros erros
      console.error('Erro ao criar responsável:', err);
      return res.status(500).json({
        message: 'Ocorreu um erro ao cadastrar o responsável. Tente novamente ou entre em contato com o suporte.'
      });
    }
  } catch (error) {
    console.error(error);
