...
export async function EnvioDeSolicitacoes(req: Request, res: Response) {
    ...
    try {
        ...
        if (solicitanteProfId !== undefined) {
            ...
            if (!solicitanteProf) {
                return res.status(404).json({ error: 'Profissional solicitante não encontrado (id fornecido não corresponde a profissional nem a usuário).' });
            }
        }
        ...
        if (solicitadoProfId !== undefined) {
            ...
            if (!solicitadoProf) {
                return res.status(404).json({ error: 'Profissional solicitado não encontrado (id fornecido não corresponde a profissional nem a usuário).' });
            }
        }
