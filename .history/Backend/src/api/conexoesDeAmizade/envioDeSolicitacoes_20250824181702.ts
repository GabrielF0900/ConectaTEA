    ...
    try {
        ...
        if (solicitanteProfId !== undefined) {
            ...
            if (!solicitanteProf) {
                ...
                if (!solicitanteProf) {
                    console.warn('[EnvioDeSolicitacoes] solicitanteProf não encontrado para id:', solicitanteProfId);
                    return res.status(404).json({ error: 'Profissional solicitante não encontrado (id fornecido não corresponde a profissional nem a usuário).', provided: { solicitanteProfId } });
                }
        }
        ...
        if (solicitadoProfId !== undefined) {
            ...
            if (!solicitadoProf) {
                ...
                if (!solicitadoProf) {
                    console.warn('[EnvioDeSolicitacoes] solicitadoProf não encontrado para id:', solicitadoProfId);
                    return res.status(404).json({ error: 'Profissional solicitado não encontrado (id fornecido não corresponde a profissional nem a usuário).', provided: { solicitadoProfId } });
                }
        }
        ...
        if (!solicitadoProf) {
            ...
            if (!solicitanteProf || !solicitadoProf) {
                ...
                console.warn('[EnvioDeSolicitacoes] falha ao resolver profissionais, faltando:', missing);
